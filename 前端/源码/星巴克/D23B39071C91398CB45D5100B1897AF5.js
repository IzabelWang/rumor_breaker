...
Object.defineProperty(exports, "__esModule", {
    ...
    make: function() {
        if (this.typeNumber < 1) {
            var t = 1;
            for (t = 1; t < 40; t++) {
                for (var e = h.getRSBlocks(t, this.errorCorrectLevel), r = new l(), o = 0, n = 0; n < e.length; n++) o += e[n].dataCount;
                for (n = 0; n < this.dataList.length; n++) {
                    var a = this.dataList[n];
                    r.put(a.mode, 4), r.put(a.getLength(), i.getLengthInBits(a.mode, t)), a.write(r);
                }
    ...
    setupPositionAdjustPattern: function() {
        for (var t = i.getPatternPosition(this.typeNumber), e = 0; e < t.length; e++) for (var r = 0; r < t.length; r++) {
            var o = t[e], n = t[r];
            if (null == this.modules[o][n]) for (var a = -2; a <= 2; a++) for (var s = -2; s <= 2; s++) this.modules[o + a][n + s] = -2 == a || 2 == a || -2 == s || 2 == s || 0 == a && 0 == s;
        }
    },
    ...
    setupTypeInfo: function(t, e) {
        ...
        for (n = 0; n < 15; n++) a = !t && 1 == (o >> n & 1), n < 8 ? this.modules[8][this.moduleCount - n - 1] = a : n < 9 ? this.modules[8][15 - n - 1 + 1] = a : this.modules[8][15 - n - 1] = a;
        this.modules[this.moduleCount - 8][8] = !t;
    },
    mapData: function(t, e) {
        for (var r = -1, o = this.moduleCount - 1, n = 7, a = 0, s = this.moduleCount - 1; s > 0; s -= 2) for (6 == s && s--; ;) {
            for (var u = 0; u < 2; u++) if (null == this.modules[o][s - u]) {
                var h = !1;
                a < t.length && (h = 1 == (t[a] >>> n & 1)), i.getMask(e, o, s - u) && (h = !h), 
                this.modules[o][s - u] = h, -1 == --n && (a++, n = 7);
            }
    ...
    for (var n = h.getRSBlocks(t, r), a = new l(), s = 0; s < o.length; s++) {
        var u = o[s];
        a.put(u.mode, 4), a.put(u.getLength(), i.getLengthInBits(u.mode, t)), u.write(a);
    }
    var f = 0;
    for (s = 0; s < n.length; s++) f += n[s].dataCount;
    ...
    for (;!(a.getLengthInBits() >= 8 * f || (a.put(e.PAD0, 8), a.getLengthInBits() >= 8 * f)); ) a.put(e.PAD1, 8);
    return e.createBytes(a, n);
}, e.createBytes = function(t, e) {
    for (var r = 0, o = 0, n = 0, a = new Array(e.length), s = new Array(e.length), h = 0; h < e.length; h++) {
        var l = e[h].dataCount, f = e[h].totalCount - l;
        o = Math.max(o, l), n = Math.max(n, f), a[h] = new Array(l);
        for (var g = 0; g < a[h].length; g++) a[h][g] = 255 & t.buffer[g + r];
        r += l;
        var c = i.getErrorCorrectPolynomial(f), d = new u(a[h], c.getLength() - 1).mod(c);
        for (s[h] = new Array(c.getLength() - 1), g = 0; g < s[h].length; g++) {
            var m = g + d.getLength() - s[h].length;
            s[h][g] = m >= 0 ? d.get(m) : 0;
        }
    }
    var v = 0;
    for (g = 0; g < e.length; g++) v += e[g].totalCount;
    var E = new Array(v), T = 0;
    for (g = 0; g < o; g++) for (h = 0; h < e.length; h++) g < a[h].length && (E[T++] = a[h][g]);
    for (g = 0; g < n; g++) for (h = 0; h < e.length; h++) g < s[h].length && (E[T++] = s[h][g]);
    return E;
};
...
for (var r = {
    ...
    getErrorCorrectPolynomial: function(t) {
        for (var e = new u([ 1 ], 0), r = 0; r < t; r++) e = e.multiply(new u([ 1, a.gexp(r) ], 0));
        return e;
    },
...
}, s = 0; s < 8; s++) a.EXP_TABLE[s] = 1 << s;

for (s = 8; s < 256; s++) a.EXP_TABLE[s] = a.EXP_TABLE[s - 4] ^ a.EXP_TABLE[s - 5] ^ a.EXP_TABLE[s - 6] ^ a.EXP_TABLE[s - 8];
...
u.prototype = {
    ...
    multiply: function(t) {
        for (var e = new Array(this.getLength() + t.getLength() - 1), r = 0; r < this.getLength(); r++) for (var o = 0; o < t.getLength(); o++) e[r + o] ^= a.gexp(a.glog(this.get(r)) + a.glog(t.get(o)));
        return new u(e, 0);
    },
    mod: function(t) {
...
h.getRSBlocks = function(t, e) {
    ...
    for (var o = r.length / 3, n = new Array(), i = 0; i < o; i++) for (var a = r[3 * i + 0], s = r[3 * i + 1], u = r[3 * i + 2], l = 0; l < a; l++) n.push(new h(s, u));
    return n;
...
var f = function(t) {
    t = t || {}, (t = Object.assign({
        ...
        for (var o = wx.createCanvasContext && wx.createCanvasContext(t.canvasId), n = t.width / r.getModuleCount(), i = t.height / r.getModuleCount(), a = 0; a < r.getModuleCount(); a++) for (var s = 0; s < r.getModuleCount(); s++) {
            var u = r.isDark(a, s) ? t.foreground : t.background;
            o.setFillStyle(u);
            var h = Math.ceil((s + 1) * n) - Math.floor(s * n), l = Math.ceil((a + 1) * n) - Math.floor(a * n);
            o.fillRect(Math.round(s * n), Math.round(a * i), h, l);
        }
        o.draw();
